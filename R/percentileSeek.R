...
#'
  percentileSeek <- function(scores, z, desired.props, restaurant.tol = 10, max.iterations = 20, resolve.ties = FALSE) {
    ...
    gradeable.rests <- score.zip.df[which(complete.cases(score.zip.df)),]
    ...
    unique.scores.zip <- aggregate(data = score.zip.df, scores ~ z, FUN = unique)
  ...
  updateGamma <- function(scores, z, desired.props, gamma.perc, index.to.update,
    ...
    if(resolve.ties == TRUE){
      zip.cutoffs <-
        findCutoffs(scores, z, gamma = gamma.perc, type = "perc.resolve.ties")
    } else{
      zip.cutoffs <- findCutoffs(scores, z, gamma = gamma.perc, type = "perc")
    }
    ...
    if(abs(diff.aj.desired) > restaurant.tol/no.rests.graded){
      ...
      #print(paste("adjusted grade proportions (A/B/C etc): ", adj.gr.props$proportion, sep=""))
